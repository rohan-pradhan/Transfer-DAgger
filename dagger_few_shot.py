...
if __name__ == "__main__":
    ...
    parser.add_argument("--dagger_iterations", help="", default=3)
    ...
    if (args.transfer_learning == 'True'):
        ...
        args.weights_out_file = "./weights/learner_FEW_SHOT_TRANSFER_{:d}_itr_0_rc_{:s}.weights".format(args.train_layers, args.road_color)
        cumulative_rewards = []
        ...
        os.mkdir("./dataset/train_few_shot_TRANSFER_{:d}_rc_{:s}".format(args.train_layers, args.road_color))
        args.out_dir = "./dataset/train_few_shot_TRANSFER_{:d}_rc_{:s}".format(args.train_layers, args.road_color)
        args.train_dir="./dataset/train_few_shot_TRANSFER_{:d}_rc_{:s}".format(args.train_layers, args.road_color)
        ...
        for i in range(1, args.dagger_iterations):
            args.run_id = i
            args.weights_out_file = "./weights/learner_FEW_SHOT_TRANSFER_{:d}_itr_{:d}_rc_{:s}.weights".format(args.train_layers, i, args.road_color)
            cumulative_rewards.append(racer.run(policy, args))
            policy = train_policy_few_shot.main(args, policy)


    else:
        args.weighted_loss = True
        args.weights_out_file = "./weights/learner_FEW_SHOT_FULL_{:d}_rc_{:s}.weights".format(args.train_layers, args.road_color)
        cumulative_rewards = []
        ...
        args.run_id = 1000
        args.timesteps= 100000
        args.out_dir = "./dataset/train_one_shot"
        args.train_dir="./dataset/train_one_shot"
        os.mkdir("./dataset/train_one_few_FULL_rc_{:s}".format(args.road_color))
        args.out_dir = "./dataset/train_few_shot_FULL_rc_{:s}".format(args.road_color)
        args.train_dir="./dataset/train_few_shot_FULL_rc_{:s}".format(args.road_color)
